...
public class MainFrame extends JFrame {
    ...
    public MainFrame(NoticeManager manager, User user) {
        ...
        userInfoLabel = new JLabel("<html><b>사용자:</b> " + user.getName() + " | <b>관심 키워드:</b> " + user.getSubscribedKeywords() + "</html>");
        ...
        JButton refreshBtn = new JButton("새로 고침 (수집)");
    ...
    private void startCrawling(JProgressBar progressBar) {
        ...
        new Thread(() -> {
            try {
                User freshUser = Main.loadUserFromJson("user.json");
                this.user = freshUser; 

                manager.run(user);

                SwingUtilities.invokeLater(() -> {
                    userInfoLabel.setText("<html><b>사용자:</b> " + user.getName() + " | <b>관심 키워드:</b> " + user.getSubscribedKeywords() + "</html>");
                    updateTable();
                    progressBar.setVisible(false);
                    JOptionPane.showMessageDialog(this, "새로운 공지 수집이 완료되었습니다!", "수집 완료", JOptionPane.INFORMATION_MESSAGE);
                });
            } catch (Exception e) {
                SwingUtilities.invokeLater(() -> {
    ...
    private void openSettingsDialog() {
        ...
        for (String kw : user.getSubscribedKeywords()) listModel.addElement(kw);
        ...
        saveBtn.addActionListener(e -> {
            ...
            Main.saveUserToJson(user, "user.json");
